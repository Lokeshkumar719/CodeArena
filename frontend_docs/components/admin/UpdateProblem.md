# `frontend/src/components/admin/UpdateProblem.jsx`

**Source:** `frontend/src/components/admin/UpdateProblem.jsx`  
**Doc path:** `frontend_docs/components/admin/UpdateProblem.md`

# File Purpose

Admin form to load an existing problem by id and update it with the same schema as create (`CreateProblem`).
...
| Symbol | Type | Role |
|--------|------|------|
| `UpdateProblem` | default export | Update form |
| `problemSchema` | Zod | Same as CreateProblem |
...
- Hidden test cases `useFieldArray` uses `keyName: "fieldId"` (differs from CreateProblem).
