      ...
      <div style={s.main}>
        ...
        <form onSubmit={handleSubmit(onSubmit, () => toast.error("Please fix validation errors"))}>
          ...
          <div style={s.card}>
            ...
            {[
              { name: "inputFormat",  label: "Input Format",  placeholder: "Describe the input format..."  },
              { name: "outputFormat", label: "Output Format", placeholder: "Describe the output format..." },
              { name: "constraints",  label: "Constraints",   placeholder: "e.g. 1 ≤ n ≤ 10^5"            },
            ].map((field) => (
              <div style={s.formGroup} key={field.name}>
                <label style={s.label}>{field.label}</label>
                <textarea {...register(field.name)} rows={3} style={{ ...s.textarea, ...(errors[field.name] ? s.inputError : {}) }} placeholder={field.placeholder} />
                {errors[field.name] && <p style={s.errorText}>{errors[field.name]?.message}</p>}
              </div>
            ))}
            ...
            <div style={s.row}>
              <div style={{ ...s.formGroup, flex: 1 }}>
                ...
                <input type="number" min="1" {...register("timeLimit")} style={s.input} />
                {errors.timeLimit && <p style={s.errorText}>{errors.timeLimit.message}</p>}
              </div>
              <div style={{ ...s.formGroup, flex: 1 }}>
                <label style={s.label}>Memory Limit (KB)</label>
                <input type="number" min="1024" {...register("memoryLimit")} style={s.input} />
                {errors.memoryLimit && <p style={s.errorText}>{errors.memoryLimit.message}</p>}
              </div>
            </div>
          ...
          <div style={s.card}>
            ...
            <TestCaseSection title="Visible Test Cases" fields={visibleFields} append={appendVisible} remove={removeVisible} register={register} type="visibleTestCases" visible />
            <TestCaseSection title="Hidden Test Cases" fields={hiddenFields} append={appendHidden} remove={removeHidden} register={register} type="hiddenTestCases" />
          </div>
                ...
                <div key={lang.value}>
                  ...
                  <div style={{ marginTop: "14px" }}>
                    <div style={s.formGroup}>
                      ...
                      <textarea {...register(`startCode.${index}.initialCode`)} rows={7} style={{ ...s.textarea, fontFamily: "monospace" }} />
                    </div>
                    <div style={s.formGroup}>
                      <label style={s.label}>Reference Solution</label>
                      <textarea {...register(`referenceSolution.${index}.completeCode`)} rows={7} style={{ ...s.textarea, fontFamily: "monospace" }} />
                    </div>
                  </div>
                </div>
              ))}
...
function TestCaseSection({ title, fields, append, remove, register, type, visible = false }) {
  return (
    <div style={{ marginBottom: "28px" }}>
      <div style={s.sectionHeader}>
        ...
        <button
          type="button"
          style={s.addBtn}
          onClick={() => append(visible ? { input: "", output: "", explanation: "" } : { input: "", output: "" })}
        >
          + Add Case
        </button>
      </div>
...
const s = {
  ...
  card: { background: "#0c1018", border: "1px solid #1e2738", borderRadius: "16px", padding: "28px", marginBottom: "28px" },
  cardTitle: { fontSize: "16px", fontWeight: 700, marginBottom: "20px", paddingBottom: "12px", borderBottom: "1px solid #1e2738" },
  formGroup: { marginBottom: "18px" },
  label: { display: "block", fontSize: "11px", fontWeight: 700, color: "#4b5563", textTransform: "uppercase", marginBottom: "8px" },
  input: { width: "100%", background: "#080c14", border: "1px solid #1e2738", borderRadius: "10px", color: "#e2e8f0", padding: "12px 14px", outline: "none", boxSizing: "border-box" },
  textarea: { width: "100%", background: "#080c14", border: "1px solid #1e2738", borderRadius: "10px", color: "#e2e8f0", padding: "12px 14px", outline: "none", resize: "vertical", boxSizing: "border-box", lineHeight: 1.6 },
  select: { width: "100%", background: "#080c14", border: "1px solid #1e2738", borderRadius: "10px", color: "#e2e8f0", padding: "12px 14px", outline: "none" },
  inputError: { borderColor: "#ef4444" },
  errorText: { fontSize: "12px", color: "#f87171", marginTop: "6px" },
  row: { display: "flex", gap: "20px", flexWrap: "wrap" },
  tagWrap: { display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "10px" },
  tagPill: { background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: "999px", color: "#a5b4fc", fontSize: "11px", padding: "4px 10px" },
  ...
  addBtn: { background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: "8px", color: "#a5b4fc", padding: "7px 14px", cursor: "pointer" },
  ...
  codeArea: { width: "100%", background: "#080c14", border: "1px solid #1e2738", borderRadius: "10px", color: "#e2e8f0", padding: "12px 14px", marginBottom: "10px", resize: "vertical", fontFamily: "monospace", boxSizing: "border-box" },
  langBadge: { background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.25)", borderRadius: "8px", color: "#eab308", padding: "5px 12px", fontSize: "12px", fontWeight: 600 },
  ...
  submitBtn: { width: "100%", background: "rgba(99,102,241,0.15)", border: "1px solid rgba(99,102,241,0.35)", borderRadius: "12px", color: "#a5b4fc", fontSize: "15px", fontWeight: 700, padding: "15px", cursor: "pointer" },
};
