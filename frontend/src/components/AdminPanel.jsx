              ...
              <Controller
                ...
                render={({ field }) => (
                  <div style={{ ...s.formGroup, flex: 2, minWidth: "240px" }}>
                    <label style={s.label}>
                      Tags <span style={{ color: "#4b5563", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(hold Ctrl/Cmd to multi-select)</span>
                    </label>
                    <select
                      multiple
                      value={field.value || []}
                    ...
                    {field.value?.length > 0 && (
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "10px" }}>
                        {field.value.map((tag) => <span key={tag} style={s.tagPill}>{tag}</span>)}
                      </div>
                    )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {visibleFields.map((field, index) => (
                  <div key={field.id} style={s.caseCard}>
                    ...
                    <textarea {...register(`visibleTestCases.${index}.input`)} placeholder="Input" rows={3} style={{ ...s.textarea, fontFamily: "monospace", marginBottom: "8px", whiteSpace: "pre" }} />
                    <textarea {...register(`visibleTestCases.${index}.output`)} placeholder="Output" rows={2} style={{ ...s.textarea, fontFamily: "monospace", marginBottom: "8px", whiteSpace: "pre" }} />
            ...
            <div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {hiddenFields.map((field, index) => (
                  <div key={field.id} style={s.caseCard}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
                      <span style={s.caseLabel}>Hidden Case {index + 1}</span>
                      <button type="button" style={s.removeBtn} onClick={() => removeHidden(index)}>Remove</button>
                    </div>
                    <textarea {...register(`hiddenTestCases.${index}.input`)} placeholder="Input" rows={3} style={{ ...s.textarea, fontFamily: "monospace", marginBottom: "8px", whiteSpace: "pre" }} />
                    <textarea {...register(`hiddenTestCases.${index}.output`)} placeholder="Output" rows={2} style={{ ...s.textarea, fontFamily: "monospace", whiteSpace: "pre" }} />
                  </div>
                ))}
              </div>
            </div>
...
const s = {
  ...
  caseLabel: { fontSize: "12px", fontWeight: 700, color: "#4b5563", textTransform: "uppercase", letterSpacing: "0.05em" },
