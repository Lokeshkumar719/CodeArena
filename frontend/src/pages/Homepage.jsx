...
function Homepage() {
  ...
  const fetchProblems = async () => {
    try {
      const { data } = await axiosClient.get(`/problem/getAllProblems?page=${currentPage}&limit=${PAGE_LIMIT}`);
  ...
  const isProblemSolved = (problemId) => solvedProblems.some((p) => p._id === problemId);
  ...
  const filteredProblems = problems.filter((problem) => {
    ...
    const statusMatch = filters.status === "all" || (filters.status === "solved" && solved) || (filters.status === "unsolved" && !solved);
  ...
  const filterConfigs = [
    {
      ...
      options: [{ value: "all", label: "All Problems" }, { value: "solved", label: "Solved Problems" }, { value: "unsolved", label: "Unsolved Problems" }],
    },
    {
    ...
    <div style={s.page}>
      <nav style={s.navbar}>
        ...
        <div style={{ position: "relative" }}>
          <button style={s.userBtn} onClick={() => setDropdownOpen((prev) => !prev)}>
            {user?.firstName} ▾
          </button>
          ...
          {dropdownOpen && (
            <div style={s.dropdown}>
              ...
              <button onClick={handleLogout} style={{ ...s.dropdownItem, background: "transparent", border: "none", width: "100%" }}>
                🚪 Logout
              </button>
            </div>
          )}
      ...
      <div style={s.main}>
        <div style={s.filterRow}>
          {filterConfigs.map(({ key, value, options }) => (
            <select key={key} value={value} onChange={(e) => updateFilter(key, e.target.value)} style={s.select}>
              {options.map((option) => (
                <option key={option.value} value={option.value} style={{ background: "#0c1018" }}>
                  {option.label}
                </option>
              ))}
            </select>
        ...
        <div style={s.paginationInfo}>
          Showing {(currentPage - 1) * PAGE_LIMIT + 1}–{Math.min(currentPage * PAGE_LIMIT, totalProblems)} of {totalProblems} problems
        </div>

        <div style={s.pagination}>
          <button disabled={currentPage === 1} onClick={() => setCurrentPage((p) => p - 1)} style={{ ...s.pageBtn, opacity: currentPage === 1 ? 0.4 : 1 }}>
            ← Prev
          </button>
          {Array.from({ length: totalPages }, (_, i) => (
            <button key={i + 1} onClick={() => setCurrentPage(i + 1)} style={{ ...s.pageBtn, ...(currentPage === i + 1 ? s.pageBtnActive : {}) }}>
              {i + 1}
            </button>
          ))}
          <button disabled={currentPage === totalPages} onClick={() => setCurrentPage((p) => p + 1)} style={{ ...s.pageBtn, opacity: currentPage === totalPages ? 0.4 : 1 }}>
            Next →
          </button>
        </div>
      </div>
...
const s = {
  ...
  filterRow: { display: "flex", gap: "16px", marginBottom: "32px", flexWrap: "wrap" },
  select:    { background: "#0c1018", border: "1px solid #1e2738", borderRadius: "10px", color: "#9ca3af", fontSize: "14px", fontWeight: 500, padding: "10px 18px", cursor: "pointer", outline: "none", minWidth: "180px", fontFamily: "'Sora', sans-serif" },
  ...
  solvedBadge:    { background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: "999px", color: "#22c55e", fontSize: "12px", fontWeight: 700, padding: "4px 14px" },
